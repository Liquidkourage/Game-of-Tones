...
class SpotifyService {
  ...
  constructor(options) {
    ...
    /** Reused Connect wake playlist when uris leave item=none on desktop. */
  ...
  /**
   * Tiny playlist so we can play with context_uri when uris leave Connect empty (Windows desktop).
  ...
  // Always play@0 then seek. If uris leave item=none (common on Windows desktop Connect),
  // fall back to context_uri via a wake playlist — that binds the PC app when uris do not.
      ...
      if (!s.ok) {
        const err = new Error(
          'Spotify did not start audio on the locked device (uris and playlist play both left is_playing=false). ' +
            ...
            'If the Windows app stays dead, use Spotify Web Player in Tempo → Connection, or reinstall the desktop app.',
        );
