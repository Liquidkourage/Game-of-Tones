...
class SpotifyService {
  ...
  /**
   * Count total tracks and tracks marked explicit by Spotify.
   * Full track payload is required so `track.explicit` is reliable on all items.
   *
   * Very large lists used to require one API call per 100 tracks; a single ~22k list could fire
   * ~220 GET /playlists/.../items in under 30s and trip TEMPO_SPOTIFY_FAILSAFE. We use the
   * PagingObject `total` on the first /items page, then either scan fully (up to 30 pages) or
   * sample 30 pages and scale explicit count to `total` when the playlist is longer.
   */
  async getPlaylistExplicitStats(playlistId) {
    ...
    const limit = 100;
    const MAX_EXPLICIT_STAT_PAGES = 30;
    const maxFullScan = MAX_EXPLICIT_STAT_PAGES * limit;

    let p0;
    try {
      p0 = await this._fetchPlaylistItemsPage(pid, { limit, offset: 0 });
    } catch (e) {
      this._rethrowIfRateLimited(e, 'getPlaylistExplicitStats');
      throw e;
    }
    const items0 = p0.items || [];
    const pagingTotal = typeof p0.total === 'number' && p0.total >= 0 ? p0.total : null;
    if (pagingTotal === 0) {
      return { total: 0, explicitCount: 0 };
    }
    if (pagingTotal == null) {
      return this._getPlaylistExplicitStatsFromFirstPageThenBounded(pid, p0, { limit, maxPages: MAX_EXPLICIT_STAT_PAGES });
    }

    function countChunk(items) {
      let t = 0;
      let ex = 0;
      for (const item of items) {
        ...
        t++;
        if (tr.explicit === true) ex++;
      }
      return { t, ex };
    }

    if (pagingTotal > maxFullScan) {
      let { t: scanned, ex } = countChunk(items0);
      let offset = limit;
      try {
        for (let p = 1; p < MAX_EXPLICIT_STAT_PAGES; p++) {
          const page = await this._fetchPlaylistItemsPage(pid, { limit, offset });
          const ch = page.items || [];
          if (ch.length === 0) {
            return { total: scanned, explicitCount: ex };
          }
      ...
      const explicitCount = Math.round((ex / scanned) * pagingTotal);
      return { total: pagingTotal, explicitCount, estimated: true };
    }

    let total;
    let explicitCount;
    {
      const c = countChunk(items0);
      total = c.t;
      explicitCount = c.ex;
    }
    let offset = limit;
    try {
      while (true) {
        ...
        for (const item of ch) {
          ...
          if (!tr || !tr.id) continue;
          total++;
          if (tr.explicit === true) explicitCount++;
        }
        offset += limit;
        if (ch.length < limit) break;
      }
    } catch (e) {
      this._rethrowIfRateLimited(e, 'getPlaylistExplicitStats');
      throw e;
  ...
  /**
   * First /items page did not include `total` (unexpected): use first page, then same hard cap.
   */
  async _getPlaylistExplicitStatsFromFirstPageThenBounded(playlistId, p0, { limit, maxPages }) {
    const pid = String(playlistId || '').trim();
    let total = 0;
    let explicitCount = 0;
    const items0 = p0.items || [];
