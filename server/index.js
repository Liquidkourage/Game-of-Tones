...
// Song title cleaning — shared with client/src/utils/songTitleCleaner.ts
const { cleanSongTitle } = require('./songTitleCleaner');
...
async function persistPlayerDataCard(room, player, cardOrCards) {
  if (!db || !player?.clientId || !room?.id || !cardOrCards) return;
  const payload = playerBingoCards.cardPayloadForDb(cardOrCards);
  if (!payload) return;
  try {
...
function emitBingoCardsToSocket(socketId, cardsOrCard, opts = {}) {
  const list = playerBingoCards
    .asBingoCardList(cardsOrCard)
    .map((c) => playerBingoCards.ensureBingoCardIdentity({ ...c, squares: c.squares }, socketId));
  const maxCards = playerBingoCards.normalizeMaxPlayerBingoCards(
    opts.maxCards ?? opts.room?.maxPlayerBingoCards,
    1,
  );
  io.to(socketId).emit('bingo-cards', {
    cards: list,
    maxCards,
...
function maxPlayerBingoCardsForRoom(room) {
...
async function tryRestorePlayerCardFromDb(room, clientId) {
  ...
  try {
    ...
    const list = playerBingoCards.asBingoCardList(session?.card);
...
async function restoreAndEmitBingoCardForJoiner(room, socket, clientIdOpt) {
  ...
  const bySocket = playerBingoCards.asBingoCardList(room.bingoCards.get(socket.id));
  if (bySocket.length) {
    if (!bySocket.every((c) => bingoCardMatchesCurrentRoundPool(room, c))) {
      routineServerLog(
        `🔄 Skipping stale in-memory card(s) for ${player.name} — songs not in current round pool (not deleted)`,
      );
    } else {
      storePlayerBingoCards(room, socket.id, player, bySocket);
      emitBingoCardsToSocket(socket.id, bySocket, { room });
      return true;
    }
  }

  if (clientId && room.clientCards && room.clientCards.has(clientId)) {
    const existingCards = playerBingoCards.asBingoCardList(room.clientCards.get(clientId));
    if (!existingCards.length || !existingCards.every((c) => bingoCardMatchesCurrentRoundPool(room, c))) {
      const stillLive = Array.from(room.bingoCards.values()).some((v) => {
        const list = playerBingoCards.asBingoCardList(v);
        return list.some((c) => existingCards.includes(c));
      });
      ...
      for (const [pid, c] of room.bingoCards.entries()) {
        ...
        const list = playerBingoCards.asBingoCardList(c);
        if (list.length && list[0] === existingCards[0]) {
          room.bingoCards.delete(pid);
        }
      }
      storePlayerBingoCards(room, socket.id, player, existingCards);
      emitBingoCardsToSocket(socket.id, existingCards, { room });
      void persistPlayerDataCard(room, player, existingCards);
      return true;
    }
  }
  ...
  if (clientId && db) {
    ...
    if (dbCards) {
      if (!dbCards.every((c) => bingoCardMatchesCurrentRoundPool(room, c))) {
        routineServerLog(
          `🔄 Ignored stale DB card for ${player.name} — round token or pool mismatch`,
        );
      } else {
        storePlayerBingoCards(room, socket.id, player, dbCards);
        emitBingoCardsToSocket(socket.id, dbCards, { room });
        return true;
      }
    }
  }
...
async function restoreOrDealBingoCardForJoiner(room, roomId, socket, clientIdOpt, opts = {}) {
  const { allowDeal = true } = opts;
  const player = room?.players?.get(socket.id);
  if (!player || player.isHost || player.name === 'Display') return false;

  rebindBingoCardsToActivePlayers(room);
  const bound = resolveActivePlayerCards(room, player, socket.id);
  if (bound.length) {
    storePlayerBingoCards(room, socket.id, player, bound);
    emitBingoCardsToSocket(socket.id, bound, { room });
    return true;
  }

  const restored = await restoreAndEmitBingoCardForJoiner(room, socket, clientIdOpt);
  if (restored) return true;
  if (!allowDeal) return false;

  const live =
    room.gameState === 'playing' || room.gameState === 'paused_for_verification';
  const hasPool =
    (Array.isArray(room.playlistSongs) && room.playlistSongs.length > 0) ||
    (Array.isArray(room.finalizedSongOrder) && room.finalizedSongOrder.length > 0) ||
    ...
    (Array.isArray(room.playlists) && room.playlists.length > 0) ||
    (Array.isArray(room.finalizedPlaylists) && room.finalizedPlaylists.length > 0);

  if (!live || !hasPool) return false;

  routineServerLog(`🎲 Dealing card(s) after failed restore for ${player.name}`);
  const card = await generateBingoCardForPlayer(roomId, socket.id);
  return !!card;
}

function findPriorPlayerByClientId(room, clientId, excludeSocketId) {
  if (!room?.players || !clientId) return null;
  for (const [pid, p] of room.players.entries()) {
    if (pid === excludeSocketId || !p || p.isHost) continue;
...
async function replayCurrentSnippet(roomId, deviceId) {
  ...
  try {
    ...
    if (sameTrack && sp) {
      ...
      // Lean restart: play@0 then seek — never Early offset in the play body.
      await sp.startPlayback(targetDeviceId, [`spotify:track:${song.id}`], 0);
      await new Promise((r) => setTimeout(r, 500));
      if (startMs > 0) {
        await sp.seekToPosition(startMs, targetDeviceId);
      }
      routineServerLog(`✅ Bump play@0+seek completed in ${Date.now() - t0}ms`);
    } else {
...
function hostPlayerCardEmitPayload(room, playerId, cardsOrCard, playedSongs) {
  const player = room.players.get(playerId);
  const cards = playerBingoCards.asBingoCardList(cardsOrCard);
  return {
...
async function playSongAtIndex(roomId, deviceId, songIndex, options = {}) {
  ...
  try {
    ...
    // Avoid transfer(play=false) immediately before uris play — clears Now Playing on Connect.
    if (!skipStabilization) {
      try {
        const pre = await spotifyFor(roomId)._probePlayerDirect('pre-index');
        if (!(pre.deviceId === targetDeviceId && pre.hasItem)) {
    ...
    try {
      ...
      room.currentSongStartMs = startMs;
      // play@0 then seek — never put Early/Random offset in the play body.
      const startState = await spotifyFor(roomId).startPlaybackEnsuringActive(
        targetDeviceId,
        [`spotify:track:${song.id}`],
        0,
        song.id,
      );
      if (startState?._resolvedDeviceId) {
        targetDeviceId = startState._resolvedDeviceId;
        room.selectedDeviceId = targetDeviceId;
      }
      let audioOk =
        !!startState?.is_playing && startState?.item?.id === song.id;
      if (!audioOk) {
        for (let i = 0; i < 3; i++) {
          await new Promise((r) => setTimeout(r, 400));
          const st = await spotifyFor(roomId).getCurrentPlaybackState();
          if (st?.is_playing && st?.item?.id === song.id) {
            audioOk = true;
            break;
          }
          try {
      ...
      if (audioOk && startMs > 0) {
        await new Promise((r) => setTimeout(r, 500));
        try {
          await spotifyFor(roomId).seekToPosition(startMs, targetDeviceId);
        } catch (seekErr) {
          routineServerLog(`⚠️ playSongAtIndex seek failed: ${seekErr?.message || seekErr}`);
        }
      }
      // Shuffle/repeat AFTER confirmed play — settings before play can hijack Connect.
...
function clearRoundScopedRoomState(room, roomId, opts = {}) {
  if (!room) return;
  const { clearPlayerSessions = true, resetPlayerCardState = true } = opts;
  ...
  else room.bingoCards = new Map();
  if (room.clientCards) room.clientCards.clear();
  else room.clientCards = new Map();
  room.playerDataRoundToken = null;

  if (clearPlayerSessions && roomId) {
...
function bingoCardMatchesCurrentRoundPool(room, card) {
  if (!card?.squares?.length || !room) return false;
  const poolIds = currentRoundPoolIdSet(room);
...
/** Resolve a player's live card list from socket map, player object, or stable clientId cache. */
function resolveActivePlayerCards(room, player, playerId) {
  if (!room || !player || player.isHost || player.name === 'Display') return [];

  const bySocket = playerBingoCards.asBingoCardList(room.bingoCards?.get(playerId));
  if (bySocket.length && bySocket.every((c) => bingoCardMatchesCurrentRoundPool(room, c))) {
    return bySocket;
  }

  const byPlayer = playerBingoCards.asBingoCardList(player.bingoCards || player.bingoCard);
  if (byPlayer.length && byPlayer.every((c) => bingoCardMatchesCurrentRoundPool(room, c))) {
    return byPlayer;
  }

  const clientId = player.clientId;
  if (clientId && room.clientCards?.has(clientId)) {
    const byClient = playerBingoCards.asBingoCardList(room.clientCards.get(clientId));
    if (byClient.length && byClient.every((c) => bingoCardMatchesCurrentRoundPool(room, c))) {
      return byClient;
    }
  }
...
/** Resolve primary (first) card — legacy callers. */
function resolveActivePlayerCard(room, player, playerId) {
  return resolveActivePlayerCards(room, player, playerId)[0] || null;
}

/** Re-key in-memory cards onto current player socket ids (survives disconnect/reconnect). */
function rebindBingoCardsToActivePlayers(room) {
  ...
  for (const [playerId, player] of room.players.entries()) {
    ...
    const cards = resolveActivePlayerCards(room, player, playerId);
    if (!cards.length) continue;

    storePlayerBingoCards(room, playerId, player, cards);
    rebound++;
  }
  return rebound;
}

function buildHostPlayerCardsData(room) {
  rebindBingoCardsToActivePlayers(room);
  ...
  if (room?.players) {
    for (const [playerId, player] of room.players.entries()) {
      ...
      const cards = resolveActivePlayerCards(room, player, playerId);
      if (!cards.length) continue;
      playerCardsData[playerId] = hostPlayerCardEmitPayload(room, playerId, cards, playedSongs);
    }
  }
...
function refreshRoomRoundToken(room) {
  if (!room) return;
  room.playerDataRoundToken = playersStore.buildRoundTokenFromRoom(room);
}

function persistAllDealtCardsRoundToken(room) {
  if (!room?.players || !room.bingoCards) return;
  refreshRoomRoundToken(room);
  for (const [pid, player] of room.players.entries()) {
    if (!player || player.isHost || player.name === 'Display') continue;
    const cards = playerBingoCards.asBingoCardList(room.bingoCards.get(pid));
    if (cards.length) void persistPlayerDataCard(room, player, cards);
  }
}
...
/**
 * Show/hide the night winners board on the projector.
 * Default: stay up until the call board updates (start-game / set-round) — no timed auto-hide.
 */
function setRoomNightBoardVisible(room, roomId, visible, opts = {}) {
  ...
  // Only schedule auto-hide when explicitly requested (legacy / rare).
  const autoHideMs = opts.autoHideMs;
  if (autoHideMs === false || autoHideMs == null) return;
  const ms =
...
/** After round win: drop the winner card and pin Tonight's winners until the call list updates. */
function revealWinnersBoardBetweenRounds(room, roomId) {
  if (!room || !roomId) return;
  room.lastDisplayWinner = null;
  io.to(roomId).emit('public-winner-dismissed');
  setRoomNightBoardVisible(room, roomId, true);
  routineServerLog(`🏆 Winners board revealed between rounds for room ${roomId}`);
}

/** Call list is taking over the projector — clear card + winners list. */
function clearProjectorWinnerOverlays(room, roomId) {
  if (!room || !roomId) return;
  room.lastDisplayWinner = null;
  clearNightBoardAutoHideTimer(room);
  const wasBoard = room.showNightBoard === true;
  room.showNightBoard = false;
  io.to(roomId).emit('public-winner-dismissed');
  if (wasBoard) {
    io.to(roomId).emit('night-board-visibility', {
...
/** True only while the live round is actively advancing songs (not paused/ended/waiting). */
function roomStillPlaying(roomId) {
  const room = rooms.get(roomId);
...
/**
 * Connect accepted play (2xx) but Now Playing stayed empty — freeze the round so we never
 * advance the call list on silence. Host can Skip / re-transfer / Start Game again.
 */
function haltRoomOnSilentSpotifyPlayback(roomId, room, message) {
  if (!room) return;
  stopLiveRoundTimers(roomId, room);
  room.gameState = 'paused';
  const msg =
    message ||
    'Spotify Connect accepted play but Now Playing stayed empty on the locked device. Open Spotify on that speaker, re-select it under Connection, then Start Game or Skip.';
  try {
    io.to(roomId).emit('playback-error', {
      message: msg,
  ...
  console.warn(`🛑 Room ${roomId} paused — Spotify audio not confirmed (no silent advance)`);
}
...
function haltSiblingLiveRoomsForHost(activeRoomId, ownerUserId, reason) {
  ...
  const uid = Number(ownerUserId);
  for (const [rid, r] of rooms.entries()) {
    if (rid === activeRoomId) continue;
    if (r?.ownerUserId == null || Number(r.ownerUserId) !== uid) continue;
    const gs = r.gameState;
    if (gs !== 'playing' && gs !== 'paused_for_verification' && gs !== 'paused') continue;
    routineServerLog(
...
async function pauseSpotifyForRoom(roomId, room) {
  const deviceId = room?.selectedDeviceId || loadSavedDeviceForRoom(roomId)?.id;
  if (!deviceId) return;
  try {
...
async function resolveRoomSpotifyPlaybackDeviceId(roomId, room, explicitDeviceId, state) {
  ...
  const saved = loadSavedDeviceForRoom(roomId);
  if (!target && saved?.id) target = saved.id;
  target = await spotifyDevices.resolveRoomPlaybackDeviceId(room, target, spotifyFor(roomId), state);
  // Prefer currently active Connect device when it matches the locked name, or when the
  // locked id is missing from the device list (desktop IDs rotate).
  if (!spotifyDevices.isVenueSpotifyJamMode(room)) {
    try {
      const sp = spotifyFor(roomId);
      const devices = await sp.getUserDevices();
      const nameHint = saved?.name || null;
      const picked = sp._pickConnectDeviceId(devices, target, nameHint);
      if (picked && picked !== target) {
        routineServerLog(
          `🎵 Preferring Connect device ${picked} over ${target || 'none'} (active/name match)`,
        );
        target = picked;
      }
    } catch (_) {
...
async function playNextSongSimple(roomId, deviceId, options = {}) {
  ...
  let resolvedDeviceId = await resolveRoomSpotifyPlaybackDeviceId(roomId, room, deviceId, null);
  if (!roomStillPlaying(roomId)) return;
  if (spotifyDevices.isVenueSpotifyJamMode(room)) {
  ...
  try {
    ...
    if (room.temporaryPlaylistId) {
      ...
      const startState = await spPlay.startPlaybackEnsuringActive(
        resolvedDeviceId,
      ...
      if (startState?._resolvedDeviceId) {
        resolvedDeviceId = startState._resolvedDeviceId;
        room.selectedDeviceId = resolvedDeviceId;
      }
    }
    ...
    try {
      for (let i = 0; i < 4; i++) {
        ...
        // Empty Now Playing must NOT count as "correct" (!currentId used to mask silence).
        const correctTrack = currentId === nextSong.id;
        ...
        routineServerLog(
          `🔎 Simple playback verify ${i + 1}/4: is_playing=${playing} correct_track=${correctTrack} item=${currentId || 'none'}`,
        );
        try {
          if (room.temporaryPlaylistId) {
            ...
            await spPlay.startPlaybackEnsuringActive(
              resolvedDeviceId,
              [`spotify:track:${nextSong.id}`],
              room.currentSongStartMs || 0,
              nextSong.id,
    ...
    if (!audioConfirmed && roomStillPlaying(roomId)) {
      console.warn(
        `⚠️ Call #${room.currentSongIndex + 1} on board but Spotify audio not confirmed — pausing (no silent advance)`,
      );
      haltRoomOnSilentSpotifyPlayback(
        roomId,
        room,
        'TEMPO called the song, but Spotify did not confirm audio on the locked speaker. Open Spotify on that device, re-transfer under Connection, then Skip or Resume.',
      );
      return;
    }
    ...
    if (roomStillPlaying(roomId) && !room._spotifyAdvanceLocked) {
      try {
        if (!spErr.isQuarantined()) {
          ...
          try {
            // Use ensuring path (play@0 then seek) — never raw play with large position_ms.
            await spErr.startPlaybackEnsuringActive(
              resolvedDeviceId,
              [`spotify:track:${nextSong.id}`],
              room.currentSongStartMs || 0,
              nextSong.id,
    ...
    try {
      ...
      if (needsCorrection) {
        ...
        try {
          ...
          // Restart intended track at play@0 then seek — large position_ms in play body empties Connect.
          ...
          if (room.temporaryPlaylistId && room.currentSongIndex !== undefined) {
            ...
            await spotifyFor(roomId).startPlaybackFromPlaylist(deviceId, room.temporaryPlaylistId, room.currentSongIndex, 0);
          } else {
            await spotifyFor(roomId).startPlayback(deviceId, [`spotify:track:${expectedId}`], 0);
          }
      ...
      if (attempts === 1) {
        ...
        try {
          ...
          if (currentExpectedId) {
            ...
            try { await spotifyFor(roomId).transferPlayback(deviceId, false); } catch {}
            try { await spotifyFor(roomId).pausePlayback(deviceId); } catch {}
            await spotifyFor(roomId).startPlayback(deviceId, [`spotify:track:${currentExpectedId}`], 0);
...
function shouldRegenerateBingoCardsOnStartGame(room, playlists, freeSpace, opts = {}) {
  ...
  if (Array.isArray(deckSourceIds) && deckSourceIds.length > 0) {
    ...
    const oldKey = oldOrder.map((s) => (typeof s === 'string' ? s : s?.id)).filter(Boolean).join('|');
...
io.on('connection', (socket) => {
  ...
  socket.on('join-room', async (data) => {
    ...
    if (wantsHost && claimUid != null) {
      haltSiblingLiveRoomsForHost(roomId, claimUid, 'host-joined-new-room');
    }
    ...
    if (wantsHost && room.ownerUserId != null && db && !room.hostProjectorPrefsSeeded) {
      ...
      try {
        ...
        if (p && typeof p === 'object') {
          ...
          if (p.maxPlayerBingoCards != null) {
            room.maxPlayerBingoCards = playerBingoCards.normalizeMaxPlayerBingoCards(
    ...
    const player = {
      ...
      bingoCard: priorPlayer?.bingoCard || null,
      patternComplete: priorPlayer?.patternComplete === true,
    };
    ...
    (async () => {
      try {
        ...
        if (effectiveIsHost) {
          ...
          void ensureActivePlayersHaveCards(roomId, room).then((recovered) => {
            if (recovered > 0) {
              routineServerLog(`✅ Host reconnect: recovered ${recovered} missing player card(s)`);
            }
            sendPlayerCardUpdates(roomId, true);
          });
          emitRoomPlayersRosterToHosts(roomId, room);
        ...
        if (
          !playerBingoCards.asBingoCardList(room.bingoCards?.get(socket.id)).length &&
          !effectiveIsHost &&
  ...
  // Audience song requests for the room-level Requests meta-round.
  socket.on('song-request', (data = {}, acknowledge) => {
    const reply = (payload) => {
      if (typeof acknowledge === 'function') acknowledge(payload);
    };
    try {
      ...
      const artist =
        typeof data.artist === 'string' ? data.artist.replace(/\s+/g, ' ').trim().slice(0, 120) : '';
      if (!room || !player || player.isHost || isDisplayConnectionPlayer(player) || !title) {
        reply({ ok: false });
        return;
      }
      const now = Date.now();
      if (socket.lastSongRequestAt && now - socket.lastSongRequestAt < 3000) {
        reply({ ok: false });
        return;
      }
  ...
  socket.on('moderate-song-request', (data = {}, acknowledge) => {
    const reply = (payload) => {
      if (typeof acknowledge === 'function') acknowledge(payload);
    };
    try {
      ...
      if (!room || !isHost || !status || typeof data.requestId !== 'string') {
        reply({ ok: false });
        return;
      }
      const requests = Array.isArray(room.songRequests) ? room.songRequests : [];
      const index = requests.findIndex((entry) => entry?.id === data.requestId);
      if (index < 0) {
        reply({ ok: false });
        return;
      }
      ...
      reply({ ok: true, request: updated });
    } catch (error) {
      console.error('moderate-song-request failed:', error?.message || error);
      reply({ ok: false });
    }
  });
  ...
  socket.on('request-printable-cards', (data = {}) => {
    void (async () => {
      try {
        ...
        const roundExport =
          data.roundExport && typeof data.roundExport === 'object' ? data.roundExport : null;
        const exportSongs = roundExport ? normalizeRoundExportSongs(roundExport.songs) : [];
        const useRoundSnapshot = exportSongs.length > 0;

        const useFreeSpace = useRoundSnapshot
          ? typeof roundExport.freeSpace === 'boolean'
            ? roundExport.freeSpace
            : !!room.freeSpaceEnabled
          : typeof data.freeSpace === 'boolean'
            ? data.freeSpace
            : !!room.freeSpaceEnabled;

        if (!useRoundSnapshot && !room.mixFinalized) {
        ...
        const minPool = useFreeSpace ? 24 : 25;
        if (useRoundSnapshot && exportSongs.length < minPool) {
          socket.emit('printable-cards-error', {
            message: `Saved round needs at least ${minPool} tracks in its snapshot to print cards.`,
          });
          return;
        }
        ...
        for (let i = 0; i < count; i++) {
          const chosen25 = useRoundSnapshot
            ? pickChosen25ForRoundExport(roundExport, useFreeSpace)
            : pickChosen25ForPrintableCard(room, useFreeSpace);
          if (!chosen25) {
            socket.emit('printable-cards-error', {
              message: useRoundSnapshot
                ? 'Could not build cards from this round snapshot. Check playlist layout (5×15 needs 15 tracks per column) or save the round again.'
        ...
        routineServerLog(
          `📄 Exported ${count} printable bingo cards for room ${roomId} (${useRoundSnapshot ? 'round snapshot' : 'live finalized pool'})`,
        );
      } catch (e) {
  ...
  // Host advances after a round win: clear winner card, show Tonight's winners until call list updates.
  socket.on('reveal-winners-board', (data = {}) => {
    try {
      const { roomId } = data;
      const room = rooms.get(roomId);
      if (!room) return;
      const isCurrentHost =
        room.host === socket.id || (room.players.get(socket.id) && room.players.get(socket.id).isHost);
      if (!isCurrentHost) return;
      revealWinnersBoardBetweenRounds(room, roomId);
    } catch (e) {
      console.error('❌ Error revealing winners board:', e?.message || e);
    }
  });
  ...
  socket.on('dismiss-public-winner', (data = {}) => {
    try {
      ...
      const isCurrentHost =
        room.host === socket.id || (room.players.get(socket.id) && room.players.get(socket.id).isHost);
      if (!isCurrentHost) return;
      clearProjectorWinnerOverlays(room, roomId);
  ...
  socket.on('set-max-player-bingo-cards', (data = {}) => {
    try {
      ...
      if (!isCurrentHost) return;
      if (room.gameState === 'playing' || room.gameState === 'paused_for_verification') {
        socket.emit('max-player-bingo-cards-updated', {
          maxCards: maxPlayerBingoCardsForRoom(room),
          rejected: true,
          reason: 'locked_while_live',
        });
        return;
      }
      const next = playerBingoCards.normalizeMaxPlayerBingoCards(maxCards, 1);
      if (maxPlayerBingoCardsForRoom(room) === next) return;
      room.maxPlayerBingoCards = next;
      io.to(roomId).emit('max-player-bingo-cards-updated', { maxCards: next });
      routineServerLog(`🃏 Max player bingo cards for room ${roomId}: ${next}`);
    } catch (e) {
      console.error('❌ Error setting max player bingo cards:', e?.message || e);
    }
  });
  ...
  socket.on('player-bingo', (data) => {
    ...
    const { cardId } = data || {};
    const playerCards = resolveActivePlayerCards(room, player, socket.id);
    let bingoCard = playerBingoCards.findBingoCardInList(playerCards, cardId);
    if (!bingoCard && playerCards.length) bingoCard = playerCards[0];
    if (!bingoCard) {
    ...
    if (validationResult.valid) {
      ...
      // CRITICAL: Use room.bingoCards list as source of truth (active card = bingo call cardId)
      const sourceCardList = playerBingoCards.asBingoCardList(
        room.bingoCards?.get(socket.id) || player.bingoCards || player.bingoCard,
      );
      const sourceCard =
        playerBingoCards.findBingoCardInList(sourceCardList, bingoCard?.cardId || player.bingoCard?.cardId) ||
        bingoCard ||
        sourceCardList[0] ||
        player.bingoCard ||
        null;
      ...
      // Use room.bingoCards list as source of truth
      const sourceCardListFail = playerBingoCards.asBingoCardList(
        room.bingoCards?.get(socket.id) || player.bingoCards || player.bingoCard,
      );
      const sourceCard =
        playerBingoCards.findBingoCardInList(sourceCardListFail, bingoCard?.cardId || player.bingoCard?.cardId) ||
        bingoCard ||
        sourceCardListFail[0] ||
        player.bingoCard ||
        null;
      if (!sourceCard) {
        socket.emit('bingo-result', { success: false, reason: 'Card data not found' });
  ...
  socket.on('verify-bingo', (data) => {
    ...
    if (approved) {
      ...
      // Approval shows the winner card only — winners list waits until the host advances.
      clearNightBoardAutoHideTimer(room);
      room.showNightBoard = false;
  ...
  socket.on('sync-state', async (data = {}) => {
    try {
      ...
      if (socket.id === room.host || syncingPlayer?.isHost) {
        ...
        payload.songRequests = Array.isArray(room.songRequests) ? room.songRequests : [];
      }
  ...
  socket.on('update-player-preferences', (data = {}) => {
    try {
      const { roomId, clientId, playerPreferences } = data;
      const room = rooms.get(roomId);
      if (!room) return;
      const player = room.players.get(socket.id);
      if (!player || player.isHost || player.name === 'Display') return;
      if (clientId && player.clientId && player.clientId !== clientId) return;
      void recordPlayerDataJoin(room, player, playerPreferences);
    } catch (e) {
      console.error('❌ update-player-preferences:', e?.message || e);
    }
  });
  ...
  socket.on('start-game', async (data) => {
    const {
      ...
      customMask: incomingCustomMask,
      patternComposite: incomingPatternComposite,
      linesRequired: incomingLinesRequired,
      customMatchReverse: incomingCustomReverse,
      customMatchAllowRotation: incomingCustomRot,
      customMatchAllowMirror: incomingCustomMir,
      customPatternName: incomingCustomPatternName,
      freeSpace,
      savedRoundPlayback,
      savedRoundId,
      roundName: incomingRoundName,
      prize: incomingRoundPrize,
    } = data || {};
    const firstSongSummary =
      Array.isArray(songList) && songList[0]
        ? `${songList[0].name || '?'} (${songList[0].id || 'n/a'})`
        : Array.isArray(lockedPlayOrder) && lockedPlayOrder[0]
          ? `${lockedPlayOrder[0].name || '?'} (${lockedPlayOrder[0].id || 'n/a'})`
          : 'n/a';
    routineServerLog(
      `🎮 Start game room=${roomId} playlists=${Array.isArray(playlists) ? playlists.length : 0}` +
        ` songList=${Array.isArray(songList) ? songList.length : 0}` +
        ` lockedOrder=${Array.isArray(lockedPlayOrder) ? lockedPlayOrder.length : 0}` +
        ` first=${firstSongSummary} device=${deviceId || 'none'} snippet=${snippetLength}s`,
    );
    const room = rooms.get(roomId);
    
    routineServerLog(
      `🔍 start-game host check room=${!!room} hostMatch=${room?.host === socket.id}` +
        ` rooms=${rooms.size}`,
    );
    ...
    if (room && isCurrentHost) {
      ...
      room.playlists = playlists;
        ...
        if (forceRegenerateCards) {
          clearRoundScopedRoomState(room, roomId, {
            clearPlayerSessions: true,
            resetPlayerCardState: false,
          });
      ...
      routineServerLog(
        `🔍 start-game reject socket=${socket.id} roomExists=${!!room} gameState=${room?.gameState || 'n/a'}`,
      );
  ...
  socket.on('reset-game', async (data) => {
    ...
    try {
      ...
      io.to(roomId).emit('game-restarted', {
        roomId,
        clearCard: true,
        message: 'Round cleared — waiting for next start',
      });
      routineServerLog(`🔁 Game reset for room ${roomId} (full pool/card cache cleared)`);
    } catch (e) {
  ...
  socket.on('mark-square', (data) => {
    const { roomId, songId, position, cardId } = data || {};
    const room = rooms.get(roomId);
    
    if (room) {
      const player = room.players.get(socket.id);
      const cards = resolveActivePlayerCards(room, player, socket.id);
      const roomCard =
        playerBingoCards.findBingoCardInList(cards, cardId) ||
        playerBingoCards.findBingoCardInList(room.bingoCards?.get(socket.id), cardId) ||
        player?.bingoCard ||
        null;
      if (player && roomCard) {
        const card = roomCard;
        ...
        if (square && (isFree || square.songId === songId)) {
          ...
          if (validationResult.valid && !player.patternComplete) {
            ...
            // Reset only if none of the player's cards still complete the pattern
            const stillComplete = (cards.length ? cards : [card]).some(
              (c) => validateBingoForPattern(c, room).valid,
            );
            if (!stillComplete) {
              player.patternComplete = false;
  ...
  /** Host "Set round" (pre-start): re-broadcast the finalized pool layout (column headers /
   *  call grid) and drop the splash so the projector shows the real call list — with this
   *  round's playlists as column names — before any music plays. */
  socket.on('display-set-round', (data) => {
    ...
    emitPublicDisplayPoolLayout(roomId, room);
    io.to(roomId).emit('display-hide-splash');
    routineServerLog(`🎬 Set round: call list shown on display for room ${roomId}`);
  });
...
async function generateBingoCards(roomId, playlists, songOrder = null, options = {}) {
  ...
  for (const [playerId, player] of room.players) {
    try {
      ...
      // Build N cards for this player (dealt at start; N = room.maxPlayerBingoCards)
      const cardsToDeal = maxPlayerBingoCardsForRoom(room);
      const playerCardList = [];
      for (let dealIdx = 0; dealIdx < cardsToDeal; dealIdx++) {
        let pick25 = chosen25;
        if (dealIdx > 0) {
          pick25 = [];
          if (mode === '1x75') {
            const base = (rooms.get(roomId)?.oneBySeventyFivePool || []).map(x => perListGloballyUnique[0].songs.find(sg => sg.id === x.id)).filter(Boolean);
            if (base.length < songsNeededPerCard) break;
            pick25 = properShuffle(base).slice(0, songsNeededPerCard);
          } else if (mode === '5x15') {
            const used = new Set();
            const columns = [];
            let ok = true;
            for (let col = 0; col < 5; col++) {
              ...
              const pool = properShuffle(perListGloballyUnique[col].songs);
              const colPicks = [];
              for (const sng of pool) {
                if (!used.has(sng.id)) { colPicks.push(sng); used.add(sng.id); }
                if (colPicks.length === need) break;
              }
              if (colPicks.length < need) { ok = false; break; }
              columns.push(colPicks);
            ...
            for (let row = 0; row < 5; row++) {
              for (let col = 0; col < 5; col++) {
                ...
                if (useFreeSpace && col === 2) {
                  const idxInCol = row < 2 ? row : row - 1;
                  pick25.push(columns[2][idxInCol]);
                } else {
                  pick25.push(columns[col][row]);
                }
              }
            }
          } else {
            const pool = buildGlobalPool();
            if (pool.length < songsNeededPerCard) break;
            pick25 = properShuffle(pool).slice(0, songsNeededPerCard);
          }
        }

        const card = {
          id: playerId,
          cardId: playerBingoCards.newBingoCardId(),
          squares: [],
        };
        let idx = 0;
        for (let row = 0; row < 5; row++) {
          for (let col = 0; col < 5; col++) {
            ...
            card.squares.push({
              ...
              song.youtubeRawTitle.trim() !== ''
                ? { youtubeRawTitle: song.youtubeRawTitle.trim() }
                : {}),
              ...(song.youtubeMusic === true && song.catalogDisplayVerified ? { catalogDisplayVerified: true } : {}),
              marked: false
            });
          }
        }
        ...
        routineServerLog(`✅ Generated card ${dealIdx + 1}/${cardsToDeal} for ${player.name} with ${uniqueOnCard.size} unique songs (mode=${mode})`);
        playerCardList.push(card);
      }

      if (!playerCardList.length) {
        continue; // Skip this player
      }

      routineServerLog(`🎲 Card generation method: ${mode === '5x15' ? '5x15 column-based' : mode === '1x75' ? '1x75 pool-based' : 'global pool'} (dealt ${playerCardList.length})`);

      if (!room.bingoCards) room.bingoCards = new Map();
      storePlayerBingoCards(room, playerId, player, playerCardList);
      cards.set(playerId, playerCardList);
      emitBingoCardsToSocket(playerId, playerCardList, { isNewCard: true, room });
      void persistPlayerDataCard(room, player, playerCardList);
    } catch (e) {
...
function normalizeRoundExportSongs(songs) {
...
/** Printable cards from a saved round snapshot (pre-show PDF — not the live room pool). */
function pickChosen25ForRoundExport(exportSpec, useFreeSpaceOpt) {
  const useFreeSpace =
    typeof useFreeSpaceOpt === 'boolean' ? useFreeSpaceOpt : !!exportSpec?.freeSpace;
  const songsNeededPerCard = useFreeSpace ? 24 : 25;
  const songs = normalizeRoundExportSongs(exportSpec?.songs);
  const geometry = String(exportSpec?.mixGeometry || 'merged');
  const playlistIds = Array.isArray(exportSpec?.playlistIds) ? exportSpec.playlistIds : [];

  if (songs.length < songsNeededPerCard) return null;

  if (geometry === '5x15' && playlistIds.length === 5) {
    const perListGloballyUnique = playlistIds.map((pid) => {
...
/**
 * Build one additional bingo card for a player (same geometry as generateBingoCardForPlayer),
 * append to their list, emit, and persist. Used to fill dealt-at-start N for late join.
 */
async function appendBingoCardsUntilCap(roomId, playerId) {
  const room = rooms.get(roomId);
  if (!room) return null;
  const p = room.players.get(playerId);
  const existing = resolveActivePlayerCards(room, p, playerId);
  const maxCards = maxPlayerBingoCardsForRoom(room);
  if (existing.length >= maxCards) {
    emitBingoCardsToSocket(playerId, existing, { room });
    return existing[0] || null;
  }
  // generateBingoCardForPlayer builds one fresh card and replaces or appends via options.
  return generateBingoCardForPlayer(roomId, playerId, { append: true, fillToMax: true });
}

// Generate bingo card(s) for one player (late join / recovery). Deals up to room.maxPlayerBingoCards.
async function generateBingoCardForPlayer(roomId, playerId, options = {}) {
  const room = rooms.get(roomId);
  if (!room) return;
  ...
  try {
    ...
    const existing = resolveActivePlayerCards(room, p, playerId);
    const maxCards = maxPlayerBingoCardsForRoom(room);
    const append = options.append === true || existing.length > 0;
    let nextList;
    ...
    if (options.fillToMax !== false && nextList.length < maxCards) {
      while (resolveActivePlayerCards(room, p, playerId).length < maxCards) {
        const before = resolveActivePlayerCards(room, p, playerId).length;
...
async function startAutomaticPlayback(roomId, playlists, deviceId, songList = null) {
  ...
  try {
    ...
    let audioConfirmed = false;
    try {
      // Resolve device if locked id is gone — do NOT transfer(play=false) before first play.
      // That clears Now Playing and pairs badly with Connect recovery; ensuring loads play@0.
      {
        const spPlay = spotifyFor(roomId);
        ...
        if (!deviceInList) {
          const saved = loadSavedDeviceForRoom(roomId);
          const retarget = spPlay._pickConnectDeviceId(devices, targetDeviceId, saved?.name);
          if (retarget && retarget !== targetDeviceId) {
            routineServerLog(`🎵 Locked id missing — retargeting to ${retarget}`);
            targetDeviceId = retarget;
            room.selectedDeviceId = targetDeviceId;
      ...
      // Always play@0 via ensuring; seek Early offset only after audio is confirmed.
      try {
        const pre = await spotifyFor(roomId)._probePlayerDirect('pre-first');
        const onTarget = pre.deviceId && pre.deviceId === targetDeviceId;
        if (!onTarget || !pre.hasItem || pre.statusCode === 204) {
          // Empty/204 after end-all: wake with play=true (never play=false before first uris play).
          routineServerLog('🔀 First-song wake transfer(play=true) — player empty or not on target');
          try {
            await spotifyFor(roomId).transferPlayback(targetDeviceId, true);
          } catch (wakeErr) {
            routineServerLog(`⚠️ First-song wake transfer failed: ${wakeErr?.message || wakeErr}`);
          }
          await new Promise((r) => setTimeout(r, 700));
        } else {
          routineServerLog('🔒 First-song skip transfer — target already active with item');
        }
      } catch (_) {
        /* probe optional */
      }
      ...
      const startState = await spotifyFor(roomId).withRetries(
        'startPlayback(initial)',
        () =>
          spotifyFor(roomId).startPlaybackEnsuringActive(
            targetDeviceId,
      ...
      // Confirm is_playing + correct track before any progression / post-play settings.
      const spConfirm = spotifyFor(roomId);
      for (let i = 0; i < 5; i++) {
        await new Promise((r) => setTimeout(r, 450));
        const state = await spConfirm.getCurrentPlaybackState();
        const playing = !!state?.is_playing;
        const currentId = state?.item?.id;
        const correctTrack = currentId === firstSong.id;
        routineServerLog(
          `🔎 First-song verify ${i + 1}/5: is_playing=${playing} correct_track=${correctTrack} item=${currentId || 'none'}`,
        );
        if (playing && correctTrack) {
          audioConfirmed = true;
          break;
        ...
        if (i < 4) {
          try {
            await spConfirm.startPlaybackEnsuringActive(
              targetDeviceId,
              [`spotify:track:${firstSong.id}`],
              0,
              firstSong.id,
            ...
            routineServerLog(`⚠️ First-song re-start failed: ${retryErr?.message || retryErr}`);
          }
        }
      }

      if (!audioConfirmed) {
        haltRoomOnSilentSpotifyPlayback(roomId, room);
        return;
      }
      ...
      if (startMs > 0) {
        ...
        try {
          await spotifyFor(roomId).seekToPosition(startMs, targetDeviceId);
          routineServerLog(`✅ First-song seek to Early offset ${startMs}ms after audio confirm`);
        } catch (seekErr) {
          routineServerLog(`⚠️ First-song seek failed: ${seekErr?.message || seekErr}`);
        }
      }

      try {
        await spotifyFor(roomId).withRetries(
          'setShuffle(false,after-confirm)',
      ...
      if (/token expired/i.test(message)) {
        ...
        try {
          ...
          if (stillMissing) {
            const saved = loadSavedDeviceForRoom(roomId);
            const retarget = spRefresh._pickConnectDeviceId(devicesAfter, targetDeviceId, saved?.name);
            if (retarget) {
              targetDeviceId = retarget;
              room.selectedDeviceId = targetDeviceId;
            } else {
          ...
          for (let i = 0; i < 4; i++) {
            await new Promise((r) => setTimeout(r, 450));
            const state = await spotifyFor(roomId).getCurrentPlaybackState();
            if (state?.is_playing && state?.item?.id === firstSong.id) {
              audioConfirmed = true;
              break;
            }
          }
          ...
          haltRoomOnSilentSpotifyPlayback(
            roomId,
            room,
            'Unable to start on locked device after token refresh. Ensure Spotify is online and try again.',
          );
          return;
        }
      } else {
        haltRoomOnSilentSpotifyPlayback(
          roomId,
          room,
          'Unable to start on locked device. Ensure it is online and try again.',
        );
        return;
      }
    }
    ...
    // Track called song only after audio is confirmed — never fake-progress on silence.
    room.calledSongIds = Array.isArray(room.calledSongIds) ? room.calledSongIds : [];
    room.calledSongIds.push(firstSong.id);
    room.currentSong = {
      id: firstSong.id,
...
async function playNextSong(roomId, deviceId) {
  ...
  if (!room || room.gameState !== 'playing' || !room.playlistSongs) {
    routineServerLog(
      `❌ Cannot play next song: exists=${!!room} state=${room?.gameState} songs=${room?.playlistSongs?.length || 0}`,
    );
    return;
  }
  ...
  try {
    ...
    // Assert playback on the locked/saved device — prefer play=true when empty; skip if already on target.
    try {
      ...
      if (needTransfer) {
        await spotifyFor(roomId).withRetries(
          'transferPlayback(next)',
          () => spotifyFor(roomId).transferPlayback(targetDeviceId, true),
          { attempts: 2, backoffMs: 300 },
        );
      }
    } catch (e) {
    ...
    try {
      ...
      if (!roomStillPlaying(roomId)) return;
      // play@0 then seek — never Early offset in the play body.
      if (room.temporaryPlaylistId) {
        routineServerLog(`🎼 Playing next song from playlist context at index ${room.currentSongIndex}`);
        await spotifyFor(roomId).withRetries(
          'startPlaybackFromPlaylist(next)',
          () =>
            spotifyFor(roomId).startPlaybackFromPlaylist(
              targetDeviceId,
              room.temporaryPlaylistId,
              room.currentSongIndex,
              0,
    ...
    // Verify playback actually started and is the correct track (play@0 retries only).
    try {
      let playing = false;
      ...
      if (playing && correctTrack && startMs > 0) {
        await new Promise((r) => setTimeout(r, 500));
        try {
          await spotifyFor(roomId).seekToPosition(startMs, targetDeviceId);
          routineServerLog(`✅ Next-song seek to Early offset ${startMs}ms after audio confirm`);
        } catch (seekErr) {
          routineServerLog(`⚠️ Next-song seek failed: ${seekErr?.message || seekErr}`);
        }
      }
      // Shuffle/repeat AFTER confirmed play
      try { await spotifyFor(roomId).setShuffleState(false, targetDeviceId); } catch (_) {}
      try { await spotifyFor(roomId).setRepeatState('off', targetDeviceId); } catch (_) {}
      if (!playing) {
        io.to(roomId).emit('playback-warning', {
