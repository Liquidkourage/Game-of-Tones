                        ...
                        <div className="call-song-info" style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                          <AnimatePresence mode="popLayout" initial={false}>
                            <motion.div
                              ...
                              transition={{ duration: 0.25 }}
                              className="call-song-name"
                              style={{ fontWeight: 900, lineHeight: 1.25, fontSize: (() => { const rh = rowHeightPx || 0; const px = Math.round(rh * 0.50); return Math.min(48, Math.max(24, px)); })() + 'px', color: '#ffffff', textShadow: '0 2px 6px rgba(0,0,0,0.8)', whiteSpace: 'normal', wordBreak: 'keep-all', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}
                            >
                              {title}
                            </motion.div>
                            <motion.div
                              ...
                              transition={{ duration: 0.25 }}
                              className="call-song-artist"
                              style={{ fontSize: (() => { const rh = rowHeightPx || 0; const px = Math.round(rh * 0.40); return Math.min(40, Math.max(18, px)); })() + 'px', color: '#e0e0e0', lineHeight: 1.14, fontWeight: 800, textShadow: '0 2px 4px rgba(0,0,0,0.6)', whiteSpace: 'normal', wordBreak: 'break-word', display: '-webkit-box', WebkitLineClamp: 1, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}
                            >
                              {artist}
                            </motion.div>
                          </AnimatePresence>
