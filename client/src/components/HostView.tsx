...
const HostView: React.FC = () => {
  ...
  const [eventTemplates, setEventTemplates] = useState<HostEventTemplate[]>([]);
  const [eventTemplatesLoading, setEventTemplatesLoading] = useState(false);
  const [eventTemplatesLoadedOnce, setEventTemplatesLoadedOnce] = useState(false);
  const [eventTemplateDraftName, setEventTemplateDraftName] = useState('');
  const [eventTemplateBusyKey, setEventTemplateBusyKey] = useState<string | null>(null);
  ...
  const loadEventTemplates = useCallback(
    async (opts?: { silent?: boolean }) => {
      const silent = opts?.silent === true;
      if (hostAccount === undefined) return;
      if (hostAccount === null) {
        setEventTemplates([]);
        setEventTemplatesLoadedOnce(true);
        return;
      }
      if (!silent) {
      ...
      try {
        const res = await hostFetch(`${API_BASE || ''}/api/host/event-templates`);
        if (!res.ok) {
          throw new Error(`HTTP ${res.status}`);
        }
        const data = (await res.json()) as { templates?: HostEventTemplate[] };
        setEventTemplates(Array.isArray(data.templates) ? data.templates : []);
        setEventTemplatesLoadedOnce(true);
      } catch (error) {
        console.error('loadEventTemplates:', error);
        if (!silent) showToast('Could not load event templates', 'error');
      } finally {
        if (!silent) setEventTemplatesLoading(false);
      }
    },
    [hostAccount],
  );
  ...
  useEffect(() => {
    setEventTemplates([]);
    setEventTemplatesLoadedOnce(false);
  }, [hostAccount?.id]);

  useEffect(() => {
    if (hostGlassNav !== 'settings') return;
    if (eventTemplatesLoadedOnce || eventTemplatesLoading) return;
    void loadEventTemplates();
  }, [hostGlassNav, eventTemplatesLoadedOnce, eventTemplatesLoading, loadEventTemplates]);

  const applyEventTemplateToPrep = useCallback(
    (template: HostEventTemplate) => {
      const hostFsDefault = readHostDefaultFreeSpaceFlag();
      const migrated = migrateRawEventRounds(template.payload?.rounds);
      const promoted = promoteRoundStatusesAfterPrepLoad(migrated, hostFsDefault);
      const nextRounds =
        promoted.length > 0
          ? promoted
          : [
              {
                ...
                songCount: 0,
                status: 'unplanned' as const,
                bingoPattern: 'line' as const,
              },
            ];
      const preferredIndex =
        typeof template.payload?.currentRoundIndex === 'number' ? template.payload.currentRoundIndex : -1;
      const nextIndex =
        preferredIndex >= 0 && preferredIndex < nextRounds.length
          ? preferredIndex
          : nextRounds.findIndex((round) => round.status === 'active');
      const prepIndex =
        nextIndex >= 0 && (nextRounds[nextIndex]?.playlistIds || []).length > 0
          ? nextIndex
          : nextRounds.findIndex((round) => (round.playlistIds || []).length > 0);
      const activeIndex = prepIndex >= 0 ? prepIndex : nextIndex;

      setEventRounds(nextRounds);
      setCurrentRoundIndex(activeIndex);
      setRoundBuilderFocusIndex(Math.max(0, activeIndex >= 0 ? activeIndex : 0));
      if (prepIndex >= 0) {
        syncMixFromRound(prepIndex, nextRounds[prepIndex]);
        applyRoundBingoToHost(nextRounds[prepIndex], { restorePlaybackFromSnapshot: true });
      }
      try {
        localStorage.setItem(`event-rounds-${roomId}`, JSON.stringify(nextRounds));
      } catch (error) {
        console.warn('Failed to save loaded template to localStorage:', error);
      }
      setHostGlassNav('rounds');
      addLog(`Loaded event template "${template.name}"`, 'info');
      showToast(`Loaded "${template.name}"`, 'success');
    },
    [roomId, addLog, syncMixFromRound, applyRoundBingoToHost],
  );

  const createEventTemplate = useCallback(async () => {
    const name = eventTemplateDraftName.trim();
    if (!name) {
      showToast('Name the template first', 'warn');
      return;
    }
    if (gameState === 'playing') {
      showToast('Finish or pause the live round before saving a template', 'warn');
      return;
    }
    setEventTemplateBusyKey('create');
    try {
      const res = await hostFetch(`${API_BASE || ''}/api/host/event-templates`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const data = (await res.json().catch(() => ({}))) as { template?: HostEventTemplate; message?: string };
      if (!res.ok) {
        throw new Error(data.message || `HTTP ${res.status}`);
      }
      setEventTemplateDraftName('');
      addLog(`Saved event template "${name}"`, 'info');
      showToast(`Saved "${name}"`, 'success');
      await loadEventTemplates({ silent: true });
    } catch (error) {
      console.error('createEventTemplate:', error);
      showToast('Could not save event template', 'error');
    } finally {
      setEventTemplateBusyKey(null);
    }
  }, [eventTemplateDraftName, gameState, eventRounds, currentRoundIndex, addLog, loadEventTemplates]);

  const updateEventTemplate = useCallback(
    async (template: HostEventTemplate) => {
      if (gameState === 'playing') {
        showToast('Finish or pause the live round before updating a template', 'warn');
        return;
      }
      setEventTemplateBusyKey(`update-${template.id}`);
      try {
        const res = await hostFetch(`${API_BASE || ''}/api/host/event-templates/${template.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            name: template.name,
        ...
        addLog(`Updated event template "${template.name}"`, 'info');
        showToast(`Updated "${template.name}"`, 'success');
        await loadEventTemplates({ silent: true });
      } catch (error) {
        console.error('updateEventTemplate:', error);
        showToast('Could not update event template', 'error');
      } finally {
        setEventTemplateBusyKey(null);
      }
    },
    [gameState, eventRounds, currentRoundIndex, addLog, loadEventTemplates],
  );

  const deleteEventTemplate = useCallback(
    async (template: HostEventTemplate) => {
      const ok = window.confirm(`Delete template "${template.name}"?`);
      if (!ok) return;
      setEventTemplateBusyKey(`delete-${template.id}`);
      try {
        const res = await hostFetch(`${API_BASE || ''}/api/host/event-templates/${template.id}`, {
          method: 'DELETE',
        });
        ...
        setEventTemplates((prev) => prev.filter((row) => row.id !== template.id));
        addLog(`Deleted event template "${template.name}"`, 'info');
        showToast(`Deleted "${template.name}"`, 'success');
      } catch (error) {
        console.error('deleteEventTemplate:', error);
        showToast('Could not delete event template', 'error');
      } finally {
        setEventTemplateBusyKey(null);
      }
    },
  ...
  const eventTemplatesPanel = (
    <section className="host-glass-panel host-settings-workspace__templates">
      <div className="host-settings-workspace__head">
        <h2 className="host-settings-workspace__title">Event templates</h2>
        <button
          type="button"
          className="btn-secondary"
          onClick={() => void loadEventTemplates()}
          disabled={eventTemplatesLoading || hostAccount === undefined}
        >
          {eventTemplatesLoading ? 'Refreshing…' : 'Refresh'}
        </button>
      </div>
      <p className="host-settings-workspace__lead">
        Save your current rounds and snapshots as a reusable show plan, then load it back into prep for repeat nights.
      </p>
      {hostAccount === undefined ? (
        <p className="host-settings-workspace__hint">Checking your Tempo account…</p>
      ) : hostAccount === null ? (
        <p className="host-settings-workspace__hint">
          Link a Tempo host account from the home screen to save templates across sessions.
        </p>
      ) : (
        <>
          <div className="host-template-library__compose">
            <label className="host-template-library__field">
              <span className="host-host-prefs__label">Template name</span>
              <input
                type="text"
                className="host-template-library__input"
                placeholder="Weekly Wednesday Music Bingo"
                value={eventTemplateDraftName}
                onChange={(e) => setEventTemplateDraftName(e.target.value)}
                maxLength={160}
              />
            </label>
            <div className="host-template-library__compose-actions">
              <button
                type="button"
                className="btn-secondary"
                onClick={() => void createEventTemplate()}
                disabled={eventTemplateBusyKey === 'create' || gameState === 'playing'}
              >
                {eventTemplateBusyKey === 'create' ? 'Saving…' : 'Save current prep as template'}
              </button>
            </div>
          </div>
          {gameState === 'playing' ? (
            <p className="host-settings-workspace__hint">
              Template load/save is disabled while a round is live so you do not clobber active prep.
            </p>
          ) : null}
          {eventTemplates.length === 0 ? (
            <p className="host-settings-workspace__hint">
              No templates yet. Save the current prep once and it will show up here.
            </p>
          ) : (
            <ul className="host-template-library__list">
              {eventTemplates.map((template) => (
                <li key={template.id} className="host-template-library__item">
                  <div className="host-template-library__meta">
                    <div className="host-template-library__name">{template.name}</div>
                    <div className="host-template-library__sub">
                      {template.roundCount} round{template.roundCount === 1 ? '' : 's'} · Updated{' '}
                      {new Date(template.updatedAt).toLocaleString()}
                    </div>
                  </div>
                  <div className="host-template-library__actions">
                    <button
                      type="button"
                      className="btn-secondary"
                      onClick={() => applyEventTemplateToPrep(template)}
                      disabled={gameState === 'playing' || eventTemplateBusyKey != null}
                    >
                      Load
                    </button>
                    <button
                      type="button"
                      className="btn-secondary"
                      onClick={() => void updateEventTemplate(template)}
                      disabled={gameState === 'playing' || eventTemplateBusyKey != null}
                    >
                      {eventTemplateBusyKey === `update-${template.id}` ? 'Updating…' : 'Update from current prep'}
                    </button>
                    <button
                      type="button"
                      className="btn-danger-outline"
                      onClick={() => void deleteEventTemplate(template)}
                      disabled={eventTemplateBusyKey != null}
                    >
                      {eventTemplateBusyKey === `delete-${template.id}` ? 'Deleting…' : 'Delete'}
                    </button>
                  </div>
                </li>
              ))}
          ...
          <div className="tab-content host-unified host-glass-workspace">
            ...
            {hostGlassNav === 'settings' ? (
              ...
              <HostSettingsPanel
                ...
                templatesPanel={eventTemplatesPanel}
